...
// W10230: slice bit 0 so the 1-bit target matches the 1-bit RHS (WIDTH==1 guard is unchanged semantically)
assign rise = (WIDTH == 1) ? (o[0] & ~stage_2[0]) : 1'b0;
assign fall = (WIDTH == 1) ? (~o[0] & stage_2[0]) : 1'b0;
always @(posedge clk)
...
// W10230: slice bit 0 so the 1-bit target matches the 1-bit RHS (WIDTH==1 guard is unchanged semantically)
assign rise = (WIDTH == 1) ? (o[0] & ~stage_3[0]) : 1'b0;
assign fall = (WIDTH == 1) ? (~o[0] & stage_3[0]) : 1'b0;
always @(posedge clk)
